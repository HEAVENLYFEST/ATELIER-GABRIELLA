...
const formulas = [
  { title: 'Essentielle', price: '250€', items: ['Liste d\'achats et quantités recommandées', 'Planche d’inspiration sur-mesure', 'Conseils décoration et mise en scène'] },
...
export default function Prestations() {
  return <PageTransition><div className="prestations-layout">
    <section className="prestations-content">
      ...
      <section className="service-list">{formulas.map((formula, index) => <motion.article className={`service-row ${formula.premium ? 'service-row--premium' : ''}`} key={formula.title} initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .2 }} transition={{ duration: .7 }}><div className="service-row__content">{formula.premium && <span className="service-row__badge">Expérience signature</span>}<p className="eyebrow">Formule {String(index + 1).padStart(2, '0')}</p><h2>{formula.title}</h2><p className="service-row__price">{formula.price}</p><span className="gold-rule" /><ul>{formula.items.map(item => <li key={item}><Check size={14} />{item}</li>)}</ul><Button to="/contact" variant={formula.premium ? 'primary' : 'outline'}>Choisir cette formule</Button></div></motion.article>)}</section>
      <motion.a href="https://www.instagram.com/ateliergabrielladesigner?igsh=M3NqZThjdml0MXNi" target="_blank" rel="noreferrer" className="instagram-box" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}><Instagram size={25} /><p>Retrouvez mes créations sur Instagram<br /><strong>@ateliergabrielladesigner</strong></p></motion.a>
    </section>
